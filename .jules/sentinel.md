...
## 2026-12-30 - [Fix Unintended Privilege Escalation in Sudoers Configuration]
**Vulnerability:** Sudoers rules for security wrapper scripts were defined with `ALL=(ALL) NOPASSWD: /usr/local/bin/conky-rkhunter-wrapper.sh`.
**Learning:** Using `ALL=(ALL)` allows the designated user to execute the script as any user on the system, including other unprivileged users, by specifying the `-u` flag (e.g., `sudo -u otheruser`). This can lead to unintended privilege escalation or lateral movement.
**Prevention:** When granting `NOPASSWD` execution rights for root-level scripts, explicitly restrict the 'Runas' user to `root` using `ALL=(root) NOPASSWD:` to ensure the user can only execute the script as root and not as arbitrary system users.
