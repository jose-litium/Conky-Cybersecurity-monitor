...
## 2026-12-29 - [Fix Insecure Data Transmission Vulnerability (HTTP)]
**Vulnerability:** External requests for public IP used `curl -s ifconfig.me`, falling back to unencrypted HTTP transmission and exposing network data to Man-in-the-Middle (MITM) interception.
**Learning:** Tools like `curl` will default to HTTP if no protocol is specified. This is dangerous for sensitive information like user public IPs, which can be spoofed or logged by unauthorized intermediaries.
**Prevention:** Always use explicit HTTPS protocols (`https://ifconfig.me`) instead of HTTP when making external requests with tools like `curl` to prevent insecure data transmission vulnerabilities.
