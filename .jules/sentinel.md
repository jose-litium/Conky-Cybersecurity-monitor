...
## 2026-06-29 - [Fix Insecure Command Output Redirect]
**Vulnerability:** Shellcheck SC2024 warned about `sudo command > file` where `sudo` doesn't affect redirects, which can cause unprivileged write errors if the file shouldn't be world-writable, or can create files owned by root that shouldn't be. Although replacing with `tee` works, we found that doing so blindly inside `if` statements can mask exit codes.
**Learning:** If `sudo command | sudo tee file` is used within an `if` block, without `set -o pipefail`, the `if` evaluates the exit code of `tee` (which is usually 0), swallowing the exit code of the actual command. This caused security scans to incorrectly report "no alerts" when the actual command failed.
**Prevention:** To fix SC2024 correctly while preserving exit codes inside conditional statements, either ensure `set -o pipefail` is active before the pipeline, or use `PIPESTATUS[0]` to explicitly check the primary command's exit code. If the user already has write access to the file (e.g., dynamically created with `mktemp` in user-owned script execution), it's often safer to stick to the standard redirect (`>`) instead of introducing `tee`.
