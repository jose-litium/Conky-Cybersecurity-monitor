...
## 2025-07-25 - [Fix Sensitive Data Leak via Incomplete File Removal]
**Vulnerability:** The `delete_temporaries` function attempted to securely remove temporary log files (e.g., `/var/log/rkhunter_warnings.txt`) using `rm -f`. However, because the `/var/log` directory is root-owned and lacks write permissions for unprivileged users, the `rm` command failed with "Permission denied". This left sensitive rootkit warnings and system state data accessible on disk.
**Learning:** In Linux, file deletion requires write and execute permissions on the parent directory, regardless of who owns the file itself. Unprivileged scripts running as a standard user cannot delete files located in root-owned directories like `/var/log`, even if the file's ownership was chowned to that user.
**Prevention:** When an unprivileged script needs to securely wipe sensitive data from a file residing in a restricted directory, it must first truncate the file contents (`: > "$file"`) if it has write permissions to the file itself, before attempting to remove it. Truncation requires only write access to the file, ensuring the sensitive content is destroyed even if the file object cannot be unlinked.
