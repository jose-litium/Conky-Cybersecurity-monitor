...
## 2026-01-20 - [Fix Command Injection via Unquoted Heredocs in Script Generation]
**Vulnerability:** The `$USER` environment variable was directly injected into generated root-level systemd scripts and sudoers files via unquoted heredocs (e.g., `readonly CONKY_USER="${USER}"`), allowing for local Command Injection and Privilege Escalation if the user's `USER` variable contained malicious commands or quotes (e.g., `USER='"; id > /tmp/pwned; "'`).
**Learning:** Even standard environment variables like `$USER` or `$HOME` cannot be trusted, as they can be manipulated by an attacker before running the script. When generating scripts that run with elevated privileges via unquoted heredocs, variables evaluated at generation time must be strictly validated or securely escaped.
**Prevention:** Implement strict input validation (e.g., regex `^[a-zA-Z0-9_-]+$`) on external variables like `$USER` at the start of the script. Additionally, as a defense-in-depth measure, use `printf '%q'` to safely escape the variable before injecting it into script generation heredocs.
