...
## 2026-12-30 - [Fix Insecure Setup of RKHunter Service and Log Handling in Translated Script]
**Vulnerability:** In `Cybersecurity-monitor-conky`, the `$USER` variable was used insecurely during systemd service creation, posing a risk of command injection and privilege escalation. Additionally, the `/var/log/rkhunter_warnings.txt` log file was configured with `chmod 644` by a root systemd service and unprivileged deletion was attempted using `rm -f`, which would fail because the user lacked write permissions on `/var/log`.
**Learning:** Security fixes applied to the main application script (e.g., `Conky_app-gui.sh`) must also be explicitly mirrored in any translated or localized variants (e.g., `Cybersecurity-monitor-conky`). Using `chmod 644` for files modified by root but needing management by an unprivileged user is ineffective; unprivileged scripts cannot delete files in root-owned directories.
**Prevention:** Validate and escape `$USER` using regex `^[a-zA-Z0-9_.-]+$` and `printf '%q'` when generating root services. Use `chown ${safe_user}:` to reassert proper ownership of state files rather than modifying permissions. In unprivileged scripts, clear state files in root directories by truncation (`: > "$file"`) rather than deletion (`rm`).
