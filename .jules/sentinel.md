...
## 2026-12-30 - [Fix DoS Vulnerability in Conky Configuration]
**Vulnerability:** Using `exec` instead of `execi` for resource-intensive scripts (like RKHunter scans) in Conky configurations.
**Learning:** `exec` runs the command on every UI update interval (e.g., every second), which for heavy commands causes severe CPU/disk thrashing, leading to a local Denial of Service (DoS).
**Prevention:** Avoid using `exec` for resource-intensive scripts or operations in Conky configurations. Instead, use `execi <interval_in_seconds>` (e.g., `execi 21600` for 6 hours) to properly cache the execution and prevent severe CPU/disk thrashing.
