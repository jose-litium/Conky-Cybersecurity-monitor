...
setup_sudoers() {
    ...
    # Create a dedicated, hardened wrapper script
...
EOF
    ...
    # Using a dedicated wrapper script instead of explicit arguments
    sudo tee "$sudoers_file" > /dev/null <<EOF
# Conky Cybersecurity Monitor - Minimal sudo permissions
# Generated by conky-cybersecurity-monitor.sh

# Allow user to run the specific wrapper script without password for monitoring only
%usergroup ALL=(ALL) NOPASSWD: $wrapper_script
EOF
...
sudo /usr/local/bin/conky-rkhunter-wrapper.sh update
sudo /usr/local/bin/conky-rkhunter-wrapper.sh propupd

# Run check and capture output
if sudo /usr/local/bin/conky-rkhunter-wrapper.sh check > "$RESULT_FILE" 2>/dev/null; then
...
/usr/local/bin/conky-rkhunter-wrapper.sh update
/usr/local/bin/conky-rkhunter-wrapper.sh propupd
/usr/local/bin/conky-rkhunter-wrapper.sh check > "$RESULT_FILE" 2>/dev/null || true
...
check_rkhunter() {
    ...
    if sudo /usr/local/bin/conky-rkhunter-wrapper.sh update && \
       sudo /usr/local/bin/conky-rkhunter-wrapper.sh propupd; then
