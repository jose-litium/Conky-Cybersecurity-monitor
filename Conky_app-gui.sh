...
cleanup() {
    ...
    rm -f /tmp/rkhunter_result.* /tmp/rkhunter_warnings*.txt "$HOME/.local/conky_app/cpu_temp.txt" 2>/dev/null || true
    exit "$exit_code"
}
...
Cmnd_Alias CONKY_SENSORS = /usr/bin/sensors
...
# Note: RKHunter checks still require password for security
...
readonly TEMP_FILE="$HOME/.local/conky_app/cpu_temp.txt"
...
delete_temporaries() {
    ...
    local -a temp_files=(
        ...
        "$HOME/.local/conky_app/cpu_temp.txt"
        "/tmp/rkhunter_result."*
...
\${color white}CPU Temp: \${color red}\${execi 10 cat \${HOME}/.local/conky_app/cpu_temp.txt 2>/dev/null | head -n1 || echo "N/A"}
