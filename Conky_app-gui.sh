...
# Validate and escape USER to prevent Command Injection in heredocs
if [[ ! "${USER:-}" =~ ^[a-zA-Z0-9_.-]+$ ]]; then
    echo "ERROR: Invalid USER environment variable. It contains unsafe characters." >&2
    exit 1
fi
printf -v SAFE_USER '%q' "${USER}"
readonly SAFE_USER
...
EOF
    ...
    if sudo visudo -c -f "$sudoers_file" &>/dev/null; then
        log "Sudoers configuration validated and applied for user $SAFE_USER."
    else
...
install_conky() {
    ...
    sudo chown "${SAFE_USER}:${SAFE_USER}" /var/log/rkhunter_warnings.txt /var/log/rkhunter_warnings_prev.txt /var/log/rkhunter_status.txt 2>/dev/null || true
